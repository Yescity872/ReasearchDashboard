    ...
    if (image0 && image0.trim() !== "") {
      try {
        const uploadResult = await cloudinary.v2.uploader.upload(image0, {
          folder: 'nearby-tourist-spots-images',
          transformation: [
...
export const updateNearbyTouristSpot = async (req, res) => {
  try {
    ...
    // Get the existing spot data first
    const existingSpot = await NearbyTouristSpot.findById(id);
    if (!existingSpot) return res.status(404).json({ message: "Nearby tourist spot not found" });
      ...
      if (payload.image0 && payload.image0.trim() !== "") {
        if (isBase64Image(payload.image0)) {
          try {
            ...
            console.error("Cloudinary upload error for image0:", uploadError);
            imageUpdates.image0 = existingSpot.image0 || "";
          }
      ...
      if (payload.image1 && payload.image1.trim() !== "") {
        if (isBase64Image(payload.image1)) {
          try {
            ...
            console.error("Cloudinary upload error for image1:", uploadError);
            imageUpdates.image1 = existingSpot.image1 || "";
          }
      ...
      if (payload.image2 && payload.image2.trim() !== "") {
        if (isBase64Image(payload.image2)) {
          try {
            ...
            console.error("Cloudinary upload error for image2:", uploadError);
            imageUpdates.image2 = existingSpot.image2 || "";
          }
    ...
    const updatedImages = [
      payload.image0 !== undefined ? payload.image0 : existingSpot.image0,
      payload.image1 !== undefined ? payload.image1 : existingSpot.image1,
      payload.image2 !== undefined ? payload.image2 : existingSpot.image2
    ...
    const updated = await NearbyTouristSpot.findByIdAndUpdate(id, payload, { new: true });
    if (!updated) return res.status(404).json({ message: "Nearby tourist spot not found" });
    res.json(updated);
  } catch (error) {
    console.error("Error updating nearby tourist spot:", error);
